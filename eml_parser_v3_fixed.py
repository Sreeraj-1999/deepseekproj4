...
{{"email_type": "...", "updates": [{{"product": "...", "stage": "...", "status": "...", "confidence": "...", "reason": "..."}}], "blockers": ["..."]}}"""
        ...
        try:
            ...
            for u in data.get('updates', []):
                ...
                reason = u.get('reason', '')
                if prod and stage in STAGE_ORDER and status in STATUS_PRIORITY:
            ...
            for b in data.get('blockers', []):
                if b:
                    result['blockers'].append({
                        'products': products,
                        'blocker': b if isinstance(b, str) else str(b)
                    })
            return result
...
class VesselTrackerV3:
    ...
    def export_html(self, prefix="", all_emails=None):
        """Export HTML report: status cards, blocker list, timeline (one-liner per email), story (full text)."""
        ts = datetime.now().strftime("%Y%m%d_%H%M%S")
        files = {}
        all_emails = all_emails or []

        for vessel, products in self.state.items():
            safe_name = re.sub(r'[^\w\s-]', '', vessel).strip().replace(' ', '_')
            fname = f"{prefix}report_{safe_name}_{ts}.html"

            # Get all emails for this vessel
            vessel_emails = []
            for r in all_emails:
                if r.get('vessel', '').lower() == vessel.lower():
                    for em in r.get('email_details', []):
                        vessel_emails.append(em)

            # Sorted history (state changes only)
            ...
            # Build a lookup: date+sender -> state changes
            change_lookup = defaultdict(list)
            for h in vessel_history:
                key = f"{h['date']}|{h['sender']}"
                change_lookup[key].append(h)
            ...
            # === STATUS CARDS (clean, no engine details) ===
            cards_html = ""
            ...
            for product in sorted_products:
                ...
                cards_html += f"""<div class="card">
                    <h3>📦 {product}</h3>
                    <table><tr><th>Stage</th><th>Status</th><th>Details</th></tr>
                        {rows}
                    </table>
                </div>\n"""
            ...
            if vessel_blockers:
                ...
                for b in vessel_blockers:
                    prods = ', '.join(b['products']) if b['products'] else 'General'
                    items += f'<div class="blocker-item"><strong>{prods}:</strong> {b["blocker"]} <span class="meta">({b["date"]}, {b["sender"]})</span></div>\n'
                blockers_html = f"""<div class="blocker-section">
                    <h2>⚠️ Active Blockers</h2>
                    {items}
                </div>"""

            # === TIMELINE (one-liner per ALL emails, not just state changes) ===
            timeline_rows = ""
            for em in vessel_emails:
                date = em['date']
                sender = em['sender']
                name = sender.get('name', 'Unknown')
                role = sender.get('role', 'unknown')
                role_icon = '🚢' if role == 'ship' else '🏢' if role == 'office' else '❓'
                etype = em.get('email_type', 'other')

                # Build summary from remarks (first remark as one-liner)
                remarks = em.get('remarks', [])
                summary = remarks[0][:120] if remarks else '(no content extracted)'

                # Add state change badges if any
                change_key = f"{date}|{name}"
                changes = change_lookup.get(change_key, [])
                change_badges = ""
                for ch in changes:
                    nb = {'done': 'done', 'in_progress': 'progress', 'pending': 'pending'}.get(ch['new_status'], 'pending')
                    nl = ch['new_status'].upper().replace('_', ' ')
                    change_badges += f' <span class="badge {nb}">{ch["product"]}/{ch["stage"].upper()} → {nl}</span>'

                # Blocker tags for this email
                blocker_tags = ""
                for bl in em.get('blockers', []):
                    bt = bl['blocker'] if isinstance(bl['blocker'], str) else str(bl['blocker'])
                    blocker_tags += f' <span class="blocker-tag">⚠ {bt}</span>'

                type_badge = f'<span class="type-badge type-{etype}">{etype.replace("_"," ")}</span>'

                timeline_rows += f"""<tr>
                    <td class="timestamp">{date}</td>
                    <td>{role_icon} {name}</td>
                    <td>{type_badge}</td>
                    <td>{summary}{change_badges}{blocker_tags}</td>
                </tr>\n"""

            # === STORY (full raw emails ascending) ===
            story_html = ""
            for em in vessel_emails:
                date = em['date']
                sender = em['sender']
                ...
                role = sender.get('role', 'unknown')
                role_icon = '🚢' if role == 'ship' else '🏢' if role == 'office' else '❓'
                role_class = 'ship' if role == 'ship' else 'office' if role == 'office' else ''
                clean = em.get('clean_text', '(no text)')
                # Escape HTML
                clean = clean.replace('&', '&amp;').replace('<', '&lt;').replace('>', '&gt;')
                prods = ', '.join(em.get('products', []))
                prod_tags = f'<span class="prod-tags">{prods}</span>' if prods else ''

                story_html += f"""<div class="email-block {role_class}">
                    <div class="email-header">
                        <span class="email-icon">{role_icon}</span>
                        <strong>{name}</strong>
                        <span class="email-date">{date}</span>
                        {prod_tags}
                    </div>
                    <div class="email-body"><pre>{clean}</pre></div>
                </div>\n"""

            # Stats
            done_count = progress_count = pending_count = 0
            for prod, stages in products.items():
                for stg, info in stages.items():
...
<head>
    ...
    <style>
        ...
        .badge {{ display: inline-block; padding: 2px 8px; border-radius: 4px; font-size: 11px; font-weight: bold; text-transform: uppercase; margin-left: 4px; }}
        ...
        .type-badge {{ display: inline-block; padding: 1px 6px; border-radius: 3px; font-size: 10px; font-weight: 600; text-transform: uppercase; }}
        .type-status_update {{ background: #d4edda; color: #155724; }}
        .type-instruction {{ background: #cce5ff; color: #004085; }}
        .type-query {{ background: #fff3cd; color: #856404; }}
        .type-issue_report {{ background: #f8d7da; color: #721c24; }}
        .type-acknowledgment {{ background: #e2e3e5; color: #383d41; }}
        .type-commitment {{ background: #d1ecf1; color: #0c5460; }}
        .type-other {{ background: #f0f0f0; color: #666; }}
        .blocker-tag {{ display: inline-block; padding: 1px 6px; border-radius: 3px; font-size: 10px; background: #fff5f5; color: #c53030; border: 1px solid #feb2b2; margin-left: 4px; }}
        .prod-tags {{ display: inline-block; padding: 1px 6px; border-radius: 3px; font-size: 10px; background: #e8f4fd; color: #0056b3; margin-left: 8px; }}
        /* Story styles */
        .email-block {{ border: 1px solid #e1e4e8; border-radius: 6px; margin-bottom: 12px; overflow: hidden; }}
        .email-block.ship {{ border-left: 4px solid #28a745; }}
        .email-block.office {{ border-left: 4px solid #0056b3; }}
        .email-header {{ background: #f8f9fa; padding: 8px 15px; font-size: 13px; display: flex; align-items: center; gap: 10px; }}
        .email-icon {{ font-size: 16px; }}
        .email-date {{ color: #666; font-size: 12px; margin-left: auto; }}
        .email-body {{ padding: 12px 15px; }}
        .email-body pre {{ white-space: pre-wrap; word-wrap: break-word; font-family: 'Segoe UI', sans-serif; font-size: 13px; margin: 0; line-height: 1.5; }}
        /* Tab navigation */
        .tabs {{ display: flex; border-bottom: 2px solid #e1e4e8; margin-bottom: 20px; margin-top: 30px; }}
        .tab {{ padding: 10px 20px; cursor: pointer; font-weight: 600; color: #666; border-bottom: 3px solid transparent; margin-bottom: -2px; }}
        .tab:hover {{ color: #0056b3; }}
        .tab.active {{ color: #0056b3; border-bottom-color: #0056b3; }}
...
<div class="container">
    ...
    <div class="tabs">
        <div class="tab active" onclick="switchTab('timeline')">📋 Timeline ({len(vessel_emails)} emails)</div>
        <div class="tab" onclick="switchTab('story')">💬 Full Story</div>
    </div>

    <div id="timeline" class="tab-content active">
        <table>
            <thead>
                <tr><th>Date</th><th>From</th><th>Type</th><th>Summary</th></tr>
            </thead>
            <tbody>
                {timeline_rows}
            </tbody>
    ...
    <footer>
        Report Generated: {datetime.now().strftime('%B %d, %Y %H:%M')} | {len(vessel_emails)} emails processed | {len(vessel_history)} state changes
    </footer>
</div>
<script>
...
class EmailPipeline:
    ...
    def process_eml(self, file_path):
        ...
        print(f"DEBUG vessel={vessel} subject={subject[:80]}")
...
def main():
    ...
    html_files = pipeline.tracker.export_html(all_emails=results)
